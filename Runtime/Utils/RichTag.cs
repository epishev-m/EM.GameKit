...
{
	public static string RelScale(float percent, object text)
	{
		return $"<size={percent}%>{text}</size>";
	}

	public static string FontSize(float fontSize, object text)
	{
		return $"<size={fontSize}>{text}</size>";
	}

	public static string VOffset(float offset, object text)
	{
	...
	public static string GrayArrow => Colored("#b5b5cb", "<sprite name=arrow>");
	...
	public static string SpriteIndent(string value, float percent)
	{
		return $"<sprite name=\"{value}\"><indent={percent}%>";
	}
		
	public static string BigSprite(string value, int percentSize, float offset)
	{
		return VOffset(offset, RelScale(percentSize, Sprite(value)));
	}
		
	public static string BigSpriteIndent(string value, int percentSize, float offset, float percent)
	{
